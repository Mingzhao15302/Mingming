...
const Store = {
  ...
  save(state) {
    try {
      ...
      Toast.show('无法保存更改，请检查浏览器存储配额。', { type: 'error' });
    }
  },
  reset() {
...
function createDefaultEntity(type, index) {
  const label = type === 'line' ? `第 ${index + 1} 产线` : `第 ${index + 1} 供料方案`;
...
const App = (() => {
  ...
  function renderDashboards() {
    ['lines', 'feeds'].forEach(group => {
      ...
      state[group].forEach(entity => {
        ...
        card.dataset.status = entity.stats.status || '待处理';
        ...
        sub.textContent = entity.stats.updatedAt ? `更新于 ${formatRelative(entity.stats.updatedAt)}` : '等待视频上传';
  ...
  function toggleSummary(entity) {
    if (refs.summaryToggle.checked) {
      ...
      Toast.show(`${entity.name} 已设为汇总来源。`);
    } else {
      state.summary = { ...SUMMARY_DEFAULT };
      Toast.show('汇总来源已恢复默认。');
    }
    save();
    refreshActiveCard();
  }
    ...
    refs.paneEditButtons.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (btn.dataset.pane === 'modules') {
          const moduleName = prompt('请输入新增模块名称（留空则跳过）');
          if (moduleName) {
            entity.modules.push({
  ...
  async function ingestVideo(file) {
    ...
    if (file.type !== 'video/mp4') {
      Toast.show('仅支持 MP4 视频文件。', { type: 'error' });
      return;
    }
    const entity = getSelectedEntity();
  ...
  function loadVideo(entity) {
    ...
    if (item.src) {
      ...
      currentVideoElement.load();
      Toast.show('请上传 MP4 视频以开始播放。', { type: 'error' });
    }
  ...
  async function capturePoster() {
    ...
    if (!item || !currentVideoElement.videoWidth) {
      Toast.show('请先播放视频再截取封面。', { type: 'error' });
      return;
    }
  ...
  function translateModuleStatus(status) {
    ...
    if (status === 'amber') return '关注';
    if (status === 'red') return '警示';
    return status;
  }
